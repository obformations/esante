package com.obadda.obsante.web.rest;

import com.obadda.obsante.config.KafkaProperties;
...
class EsanteKafkaResourceIT {
    ...
    void setup() {
        ...
        EsanteKafkaResource kafkaResource = new EsanteKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/esante-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/esante-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
